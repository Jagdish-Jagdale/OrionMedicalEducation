  ...
  useEffect(() => {
    getCountries().then((data) => {
      if (data && data.length > 0) {
        const countries = data.filter(c => c.countryCardPosition || c.countrycardpostion);
        ...
        const mapped = countries.map((c, i) => {
          const fullTitle = c.title || `MBBS IN ${c.name}`;
          let prefix = "";
          let countryName = fullTitle;
          
          if (fullTitle.toUpperCase().startsWith("MBBS IN ")) {
            prefix = fullTitle.substring(0, 8);
            countryName = fullTitle.substring(8);
          }
              ...
              {marqueeData.map((item, idx) => (
                <React.Fragment key={idx}>
                  <span className="font-bold tracking-[0.25em] text-xs uppercase font-inter flex items-center gap-1.5">
                    {item.prefix && <span className="text-black">{item.prefix}</span>}
                    <span className={item.accent}>{item.countryName}</span>
                    <span className="inline-flex items-center ml-2 h-4">
                      <img
                        src={item.flagUrl}
                        alt=""
                        className="h-4 w-6 min-w-[24px] object-contain rounded-[1px] shadow-sm"
                      />
                    </span>
                  </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 sm:gap-12 lg:gap-10">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="flex items-center gap-3">
              <a href={ensureAbsoluteUrl(contactData?.youtube) || "https://youtube.com"} target="_blank" rel="noopener noreferrer" className="w-8 h-8 bg-white/10 hover:bg-red-600 rounded-lg flex items-center justify-center transition-colors">
              ...
              <a href={ensureAbsoluteUrl(contactData?.instagram) || "https://www.instagram.com/orionmedicaleducation/"} target="_blank" rel="noopener noreferrer" className="w-8 h-8 bg-white/10 hover:bg-pink-600 rounded-lg flex items-center justify-center transition-colors">
              ...
              <a href={ensureAbsoluteUrl(contactData?.facebook) || "https://facebook.com"} target="_blank" rel="noopener noreferrer" className="w-8 h-8 bg-white/10 hover:bg-blue-600 rounded-lg flex items-center justify-center transition-colors">
          ...
          {/* Quick Links */}
          <div>
            <h4 className="font-normal text-white mb-5 text-sm uppercase tracking-wider">Quick Links</h4>
          ...
          {/* Contact */}
          <div>
            <h4 className="font-normal text-white mb-5 text-sm uppercase tracking-wider">Contact</h4>
