...
const NeuralReviewCard = ({ review, position, isLeft, style }) => {
  return (
    <div
      style={style}
      className={`${position} z-30 w-full p-2.5 lg:p-5 bg-white rounded-xl lg:rounded-3xl border border-slate-100 shadow-[0_10px_40px_rgba(0,0,0,0.04)] group hover:shadow-[0_20px_50px_rgba(59,130,246,0.1)] transition-all border-l-[3px] lg:border-l-4 ${isLeft ? 'border-l-blue-500' : 'border-l-amber-500'}`}
    >
      <div className="flex items-center gap-2 lg:gap-3 mb-1.5 lg:mb-4">
        <div className="w-7 h-7 lg:w-11 lg:h-11 rounded-full border-2 border-slate-100 p-0.5 overflow-hidden flex-shrink-0 group-hover:border-blue-400 transition-colors shadow-sm">
          <img 
        ...
        <div className="overflow-hidden">
          <h4 className="text-navy font-bold text-[10px] lg:text-sm truncate group-hover:text-blue-600 transition-colors uppercase tracking-tight lg:tracking-wider">{review.studentName || 'Student'}</h4>
          <p className="text-slate-400 text-[7px] lg:text-[10px] uppercase font-black tracking-widest leading-none mt-0.5">{review.type}</p>
        </div>
      </div>
      
      <div className="relative">
        <svg className="absolute -top-1 lg:-top-2 -left-1 w-2.5 h-2.5 lg:w-4 lg:h-4 text-slate-100" fill="currentColor" viewBox="0 0 24 24">
        ...
        <p className="text-slate-600 text-[9px] lg:text-[12px] leading-relaxed italic pl-3 lg:pl-4">
